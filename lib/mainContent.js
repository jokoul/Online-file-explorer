...
const buildMainContent = (fullStaticPath, pathname) => {
  ...
  //filter and remove .DS_Store file in our items array
  items = items.filter((element) => element !== ".DS_Store");
  //Home directory, remove project-files
  console.log("maincontent.js LINE 34 : " + pathname);
  if (pathname === "/") {
    items = items.filter(
      (element) => !["project-files", "ace-builds-master"].includes(element)
    );
  }
  console.log("maincontent.js LINE 38 : " + items);
